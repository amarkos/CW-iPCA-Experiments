...
my_generate_missing <- function(n, p,rho=.8, fix_means=0,fix_corMat=NULL, prop_mis, mechanism, scenario,myseed,randVals=NULL,ncp) {
  set.seed(myseed)
  ...
  if(is.null(fix_corMat)==T){
    
    if (scenario == "M1") {
      #Scenario 1 (M1)
      r  = rho #correlation within blocks
      c1 = 4*p/9 
      c2 = 3*p/9 
      c3 = 2*p/9 
      R1 = diag(c1)
  ...
  if(is.null(randVals)==T){
    random.normal = matrix(rnorm(p*n,0,1), nrow=p, ncol=n);
  }else{
    random.normal = matrix(randVals, nrow=p, ncol=n);
  }
  # random.normal = matrix(rnorm(p*n,0,1), nrow=p, ncol=n);
  X = U %*% random.normal
  X = t(X)
